...
#user input
playlist_link = input("enter your playlist link here, or enter 'end' to exit the program: ")

#if user types "end" then end the program
if playlist_link == ("end"):
    runScript = False
...
while runScript == True:
    ...
    for idx, track_item in enumerate(tracks):
        ...
        print(f"   Artists: {', '.join(artist_name)}")
        print(f"   Links: {', '.join(artist_urls)}")

    #user input
    playlist_link = input("If you would like to rerun the script, enter another Link. If you would like to end the script, type 'end': ")

    #stops program if user enters "end"
    if playlist_link == ("end"):
        runScript = False
        break

    #restarts program if user enters another link, or types anything else
    else:
        playlist_id = playlist_link.strip("https://open.spotify.com/playlist/")
        playlist_id = playlist_id.split("?")[0]
        print(playlist_id)
